    ...
    <div class="w-full max-w-lg px-4 md:max-w-2xl lg:max-w-3xl xl:max-w-5xl">
      ...
      <div v-else :class="searching ? 'opacity-50' : ''">
        ...
        <div class="hidden min-h-[520px] overflow-hidden rounded-lg bg-gray-700 shadow-md md:block">
          <table class="w-full text-left text-xs text-white">
            <thead class="border-b border-white/10 text-xs text-white/50 lowercase">
              <tr>
                <th class="w-64 cursor-pointer select-none px-4 py-3 hover:text-white" @click="toggleSort('email')">
                ...
                <th class="cursor-pointer select-none px-4 py-3 text-center hover:text-white" @click="toggleSort('is_active')">
                  active <span v-if="sortKey === 'is_active'" class="text-white">{{ sortAsc ? '↑' : '↓' }}</span>
                </th>
                <th class="cursor-pointer select-none px-4 py-3 text-center hover:text-white" @click="toggleSort('is_verified')">
                  verified <span v-if="sortKey === 'is_verified'" class="text-white">{{ sortAsc ? '↑' : '↓' }}</span>
                </th>
                <th class="cursor-pointer select-none px-4 py-3 text-center hover:text-white" @click="toggleSort('is_superuser')">
        ...
        <div v-if="totalPages > 1" class="mt-4 flex items-center justify-between text-xs">
          <span class="text-white/40">{{ totalCount }} users</span>
