    ...
    <div class="w-full max-w-lg px-4 min-[1920px]:max-w-400 sm:max-w-none md:max-w-3xl lg:max-w-4xl xl:max-w-5xl 2xl:max-w-7xl">
        ...
        <PageHeader class="mx-2.5" :title="expandedFolder.name">
          <template #prepend>
            <button class="cursor-pointer text-white/60 hover:text-white" @click="toggleFolder(expandedFolder)">
              <Icon name="uil:arrow-left" class="mt-2.5 text-xl" />
            </button>
          </template>
        </PageHeader>
        <TodoList />
      ...
      <template v-else>
        ...
        <div v-if="folderStore.archivedFolders.length" class="mx-2.5 mt-4 mb-2">
          ...
          <div class="mb-6 columns-1 gap-3 sm:columns-2 lg:columns-3 xl:columns-4 2xl:columns-5">
            <div
              v-for="folder in folderStore.archivedFolders"
              :key="folder.uuid"
              class="border-0.5 mb-3 flex cursor-pointer break-inside-avoid items-center justify-between rounded-lg border-white/10 bg-gray-700 px-5 py-4 shadow-md"
              @click="toggleFolder(folder)"
            >
              <div class="min-w-0 flex-1">
                <p class="truncate text-sm font-bold text-white/80 lowercase">{{ folder.name }}</p>
                <p class="text-xs text-white/30 lowercase">{{ folder.note_count }} notes</p>
              </div>
              <button
                class="ml-3 shrink-0 cursor-pointer text-xs text-white/40 lowercase hover:text-white"
